# see https://github.com/NixOS/nixpkgs/pull/51803/files
...
let
  cfg = config.networking.iwd-nm;  # changed name
  # CHANGED network manager build
  nm = pkgs.networkmanager.overrideAttrs (old: rec {
      configureFlags = pkgs.networkmanager.configureFlags ++ [ "--with-iwd" ];
  });
...
in {
  ...
  options = {
    ...
    networking.iwd-nm = {
      ...
      # CHANGED use the custom built nm package for iwd
      ...
      enableStrongSwan = mkOption {
        ...
        description = ''
          ...
          the <option>networking.iwd-nm.packages</option> option
      ...
      dynamicHosts = {
        enable = mkOption {
          ...
          description = ''
            ...
            <option>networking.iwd-nm.dns</option> option to be
            set to <literal>dnsmasq</literal>. If enabled, the directories
            defined by the
            <option>networking.iwd-nm.dynamicHosts.hostsDirs</option>
  ...
  config = mkIf cfg.enable {
    ...
    assertions = [
      { assertion = config.networking.wireless.enable == false;
        message = "You can not use networking.iwd-nm with networking.wireless";
      }
      { assertion = !dynamicHostsEnabled || (dynamicHostsEnabled && cfg.dns == "dnsmasq");
        message = ''
          To use networking.iwd-nm.dynamicHosts you also need to set
          networking.iwd-nm.dns = "dnsmasq"
        '';
      }
      { assertion = !config.networking.wireless.iwd.enable;
        message = ''
          Only one option to enable iwd is allowed: networking.wireless.iwd.enable
          and networking.iwd-nm.wifi.backend = "iwd" are mutually exclusive.
        '';
      }
    ];
